...
# ========== ЕЖЕДНЕВНАЯ РАССЫЛКА (МСК ~13:00, рандомно в течение часа) ==========
async def daily_message(context: ContextTypes.DEFAULT_TYPE):
    if not REVIEW_CHAT_ID:
        return

    # Рандомная задержка 0-59 минут чтобы было в течение часа после 13:00 МСК
    ...
    messages = [
        {
            "text": (
                ...
                "Вам нравится наше РП? Тогда оставьте свой отзыв — нам важно ваше мнение!"
            ),
        ...
        },
        {
            ...
            "keyboard": [[InlineKeyboardButton("🔗 Присоединиться", url=INVITE_LINK)]]
        },
        {
            "text": (
                "🚀 <b>Поддержите Яблочное РП!</b>\n\n"
                "Если вам нравится РП — проголосуйте за него! Голосовать могут пользователи с Telegram Premium."
            ),
            "keyboard": [[InlineKeyboardButton("⭐ Проголосовать", url=BOOST_LINK)]]
        },
        {
            "text": (
                "👑 <b>Поддержите ИМПЕРАТОРА!</b>\n\n"
                "Если вам нравится наше РП — вы можете поддержать ИМПЕРАТОРА, отправив ему подарок."
            ),
            "keyboard": [[InlineKeyboardButton("🎁 Отправить подарок", url=f"https://t.me/{SUPPORT_USERNAME.replace('@', '')}")]]
        }
    ]
    ...
    try:
        await context.bot.send_message(
            ...
            text=chosen["text"],
            reply_markup=InlineKeyboardMarkup(chosen["keyboard"]),
            parse_mode="HTML"
        )
    except Exception as e:
...
def run_bot():
    ...
    # Ежедневная рассылка (каждые 24 часа, первый запуск через 10 сек)
    try:
        app.job_queue.run_repeating(daily_message, interval=86400, first=10)
    except Exception as e:
