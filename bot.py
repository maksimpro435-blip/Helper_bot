...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "enter_rp":
        ...
        text = (
            ...
            "• плотности населения\n\n"
            "ЭПОХИ:\n"
            "Древние эпохи — племена, слабая армия, постоянные конфликты\n"
            "Средние века — феодальная система, слабая промышленность\n"
            "Индустриальная эпоха — заводы, армии с техникой, рост городов\n"
            "Современная эпоха — развитая экономика, регулярные армии\n"
            "Новейшая эпоха — наука, технологии, глобальная конкуренция\n\n"
            "━━━━━━━━━━━━━━━━━━\n\n"
...
async def get_spawn(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = "🏴 <b>Шаг 3/3:</b> Скинь <b>флаг</b> твоей страны <i>(необязательно)</i>:"
    await update.message.reply_html(text, reply_markup=InlineKeyboardMarkup(keyboard))
    return FLAG
...
async def admin_reject_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton("📏 Спавн не подходит (слишком большой/неправильный)", callback_data=f"reject_{user_id}_1")],
...
async def admin_reject_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    reasons = {
        "1": "📏 <b>Спавн не подходит.</b>\nСлишком большой, не соответствует правилам или не указан чётко. Уточни место спавна.",
        "2": "📛 <b>Название не подходит.</b>\nВыбери другое название для страны, которое соответствует сеттингу."
    }
    ...
    await query.edit_message_text(f"❌ Отказано.\nПричина {reason_code}: {reason_text}", parse_mode="HTML")
...
def run_bot():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            CHANGE_SPAWN: [
                MessageHandler(filters.PHOTO | filters.TEXT & ~filters.COMMAND, change_spawn_done)
            ],
            CHANGE_FLAG: [
